...
| 1:05–1:20 | Tap "Share proof PDF". expo-print renders a one-pager (photo + transcript + location + content hash + verify QR). Native share sheet opens. | "One-page PDF. Photo, transcript, location, content hash, verify QR." | PDF render is the slowest step (~1.5s). Narrate the value while it renders. |
