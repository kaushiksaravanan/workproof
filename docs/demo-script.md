...
| 0:40–0:50 | Type or paste the short transcript. Tap Use this — Gemini extraction fills work type / client / amounts (falls back to on-device regex if network drops). Edit inline to fix a job number, customer name, or location. | "Editable transcript on notebook paper. Gemini pulls the fields; regex catches what's missed offline." | Extract path uses `/api/vend` (CipherStack token stays server-side). Regex baseline is the fallback so nothing breaks offline. |
| 0:50–1:00 | Optional: type "English" (or Hindi, Kannada, etc.) in the Target language field and tap Translate. Gemini writes the bilingual version into the record. | "The crew talks in their language, the client reads theirs. That's the India-scale problem most proof-of-work tools skip." | Translation is a network call — if it fails the proof still saves, minus the second-language column. |
| 1:00–1:15 | Tap "Save proof". expo-crypto SHA-256 hashes the canonical bundle (record fields + photo hash + audio hash + worker wallet address + optional translation). Anchor step submits the hash to Polygon Amoy via ethers.js if a contract address is configured. | "The canonical record is hashed and signed by this install's own wallet. On-chain event's `worker` field attributes the anchor to the crew." | If no anchor address is configured, the proof stays local with a `queued:` synthetic tx id. That's expected in demo mode without a deployed contract. |
| 1:15–1:25 | Tap "Share proof PDF". expo-print renders a one-pager (photo + transcript + translation + worker wallet + content hash + Polygonscan link if anchored). Native share sheet opens. | "One-page bilingual PDF. Photo, both transcripts, worker wallet, content hash, on-chain link." | PDF render is the slowest step (~1.5s). Narrate the value while it renders. |
