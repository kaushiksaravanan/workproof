...
"Contractors and crews already take job photos on their phones — but the photos scatter across camera rolls, WhatsApp threads, and lost text messages. WorkProof captures the photo, voice memo, and job details in one 30-second flow, signs the whole bundle with a device key, and exports a one-page PDF that anyone can verify later. On-device transcription writes the report; on-chain anchoring gives the artifact a permanent timestamp. Demo runs in 90 seconds on any Android phone with Expo Go."
...
| 0:40–0:50 | Watch the transcript render on the notebook-paper surface. Edit inline to fix a job number, customer name, or location. | "Edit anything — the crew often fixes a name here." | Fonts may render as system default for a beat on cold start. Wait for the fade-in. |
